...
class Ui_EditPass(object):
    ...
    def save_changes(self):
        ...
        if not all([self.new_trip_id_num, self.new_passport, self.new_surname, self.new_name, self.new_nums_routes]):
            QMessageBox.warning(self.edit_pass, "Предупреждение", "У вас заполнены не все поля.")
            return
...
class Ui_Passengers(object):
    ...
    WHERE  
          ...
          passengers.passport LIKE '%{keyword}%' OR
